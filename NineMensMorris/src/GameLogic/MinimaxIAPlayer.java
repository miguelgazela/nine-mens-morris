...
public class MinimaxIAPlayer extends IAPlayer {
	...
	private  List<Integer> generateMoves(Board gameBoard) {
		...
		for(int i = 0; i < gameBoard.boardPositions.length; i++)  //falta para quando s� d� para mover para uma adjacente
	...
	private int[] minimax(int player, int depth,Board gameBoard) {
		List<Integer> nextMoves = generateMoves(gameBoard); // Generate possible next moves in a List of Positions.
		...
		if (nextMoves.isEmpty() || depth == 0) { // Gameover or depth reached, evaluate score
			...
			for (int pos : nextMoves) {
				...
				gameBoard.boardPositions[pos].playerOccupying = Position.NO_PLAYER;
			}
		}
